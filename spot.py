...
def getPlaylist(id: str) -> List[Track]:
    ...
    track_ids = [t['track']['id'] for t in tracks]# TODO: build a list of track_ids from the tracks
    ...
    artist_ids = [artist['id'] for artist in tracks['track']['artists']]#TODO: make a list of unique artist ids from tracks list
    ...
    for k in range(1+len(artist_ids)//50): # can only request info on 50 artists at a time!
        ...
        for a in artists_response['artists']:
            artists[a['id']] = Artist(a['id'],a['name'],a['genres']) # TODO: create the Artist for each id (see audio_info, above)


    # populate track dataclass
    trackList = [Track(id = t['track']['id'],# TODO: your code here     , \
                       name= t['track']['name'],# TODO: your code here    , \
                       artists= [Artist(a['id'],a['name'],a['genres']) for a in t['track']['artists']], # TODO: your code here , \
                       audio_features= audio_info[t['id']]) for t in tracks]
...
def artist_with_most_tracks(tracks: List[Track]) -> Tuple[Artist, int]:
    ...
    for track in tracks:
        for artist in track.artists:
            tally[artist.id] += 1
            arts[artist.id] = artist

    most_tracks_artist_id = max(tally,key=tally.get)
    most_tracks_artist = arts[most_tracks_artist_id]
    most_tracks_count = tally[most_tracks_artist_id]

    return most_tracks_artist, most_tracks_count
...
def danceability_plot(tracks:List[Track]):
    ...
    plt.scatter(df['danceability'], df['speechiness'], c=df['is_rap'])

    plt.xlabel("Danceability")
    plt.ylabel('Speechiness')
    plt.title("catter plot of dancability-speechiness with markers colored by genre: is_rap")
    #assemble a scatter plot using the audio characteristics of the songs
